...
%\begin{align} 
%P(X \le c) & = \int_{0}^{c} \frac{1}{\theta^k} \frac{1}{\Gamma(k)} x^{\alpha k- 1} e^{-\frac{x}{\theta}} dx \\ P(Y \le \log c) & = \int_{\log(0)}^{\log(c)} \frac{1}{\theta^k} \frac{1}{\Gamma(k)} \exp(y)^{\alpha k- 1} e^{-\frac{\exp(y)}{\theta}} \exp(y) dy \\ 
%\end{align}
